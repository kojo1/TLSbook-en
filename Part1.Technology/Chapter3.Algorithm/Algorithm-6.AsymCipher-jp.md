...
DSA(Digital Signature Algorithm)署名では、RSAのような巡回群を使うのではなく、二つの異なる一方向演算の組み合わせ同士で同一の値を得ることができることを利用します。
...
DSAでは、このような演算構造によって特定のメッセージに対する署名の検証を実現しています。DSAでは巡回群を使っていないため、パディングを使った改ざん防止は不要である点が特徴の一つです。
...
この標準規定の中には、RSAのためのパディングスキームについても規定されています。当初、PKCS#1 v1.5として比較的単純なスキームが規定されましたが、その後より改善された方式として、暗号復号化には最適非対称暗号パディング(OAEP: Optimal asymmetric encryption padding)、また、RSAを公開鍵署名のための利用する場合のパディングとしては確率的署名スキーム(PSS: Probabilistic signature scheme)が標準化されています。PKCS#1 v1.5も後方互換のために残されていますが、現在はこれらを使用することが推奨されています。
