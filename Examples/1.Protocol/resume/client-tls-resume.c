...
/* Print stdlib function error message */
static void print_sys_error(const char* msg, int err)
{
    if (errno)
        fprintf(stderr, "ERROR: failed %s. (errno %d)\n", msg, err);
    else
...
static SSL_SESSION* read_SESS(const char* file)
{
    FILE*              fp = NULL;
    ...
    if (fseek(fp, 0, SEEK_END) != 0) {
        print_sys_error("fseek", errno);
        goto func_exits;
    }
    if ((sz = ftell(fp)) == -1) {
        print_sys_error("ftell", errno);
        goto func_exits;
    }
    printf("%s size = %ld\n", SAVED_SESS, sz);
    rewind(fp);
    if ((buff = (unsigned char*)malloc(sz)) == NULL) {
        print_sys_error("malloc", 0);
        goto func_exits;
    }
    if (fread(buff, 1, sz, fp) != sz) {
        print_sys_error("fread", 0);
        goto func_exits;
    }
...
static void write_SESS(SSL_SESSION* sess, const char* file)
{
    FILE*              fp = NULL;
    unsigned char*     buff = NULL;
    size_t             sz;
    ...
    /* sanity check */
    if (sess == NULL || file == NULL) {
        fprintf(stderr, "ERROR : bad arguments %p %s\n", sess, file);
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if ((session = read_SESS(SAVED_SESS)) != NULL) {
        if(SSL_set_session(ssl, session) != SSL_SUCCESS) {
