...
図3-6-5はDSA署名、検証の流れを示します。セキュリティ強度の観点からはDSA署名においては適切な鍵を得る鍵生成も重要なのですが、ここではDSAの原理を理解するために生成された鍵を使って署名と検証を行う部分の流れだけをしめします。細かいアルゴリズムを別として、署名生成と検証の箱のつながりを見てみます。図の右側二つの箱が署名の生成、左側が検証部分です。任意の長さのメッセージは適切なハッシュ関数H(m)で固定長のハッシュ値に変換するものとします。
...
一方、DSAは実現においては適切な鍵生成が難しく十分な注意が必要です。鍵を解読されないためには署名毎に新しい異なる鍵を生成する必要があります。また、検証のための計算量はRSAくらべてかなり大きくなる傾向にあります。このため、整数演算の世界においてはRSA署名のほうが広く利用されています。しかし、楕円曲線暗号では巡回群は見つかっていないため、整数演算の世界のDSAと等価のアルゴリズムを楕円曲線暗号の世界で実現したECDSA, EdDSA(ECCの章で説明)が広く使われています。
