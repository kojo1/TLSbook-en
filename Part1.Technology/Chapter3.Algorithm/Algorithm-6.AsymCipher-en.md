...
### 3.7.1 Background
Public-key cryptography is a cryptographic method that allows one key to be made public by using different keys for encryption and decryption. However, today, the properties of public keys are applied in various aspects, and their fields of application are diverse. In TLS, it is mainly used as a key exchange or key sharing for solving the key delivery problem of a common key, a digital signature (public key signature) and a public key certificate based on it. In this chapter, we will move on to another section about certificates, and explain the basic concept of public keys, key exchange, and signatures.
